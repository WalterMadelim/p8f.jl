...
import Random
import Plots
# N = 40, TSP problem, using Gurobi_Lazy_cuts with MOI
# 10/03/2023
...
function my_GRB_cb_function(cb_data, cb_where::Cint)
    ...
    if cb_where == Gurobi.GRB_CB_MIPSOL # whenever a new MIP solution is found (cb_where = 4)
        ...
        if 1 < l < N # subtour exists, must add cut
            ...
            terms = similar(S,MOI.ScalarAffineTerm{Float64})
...
x = similar(d,MOI.VariableIndex) # fill(v,N,N), where v = MOI.add_variable(Gurobi.Optimizer())
objterms = similar(trues(N^2),MOI.ScalarAffineTerm{Float64})
