...
import FileIO
########################################
# 01/11/23, consider sin fuel cost
...
ki = MODELING
upd_from_optimizer = true # valid only when ki != MODELING
    ...
    for g in 1:Gs, t in 1:Ts
        if ki == MODELING
            if is_model_MOI_NL # ----------- with MOI's NonLinearPortion -----------
                Nonlinear.add_constraint(nlpor,:(sin($(sinarg[g,t][ki])) - $(absarg[g,t][ki])),MOI.EqualTo(0.))
                Nonlinear.add_constraint(nlpor,:(abs($(absarg[g,t][ki])) - $(absret[g,t][ki])),MOI.EqualTo(0.))   
...
if ki == MODELING # The formal Obj Modeling part
    ...
    if is_model_MOI_NL
        ...
        MOI.add_constraint(o, obj_function - 1. * aux_obj, MOI.LessThan(0.))
...
status, maxvio_grb = MOI.get(o,MOI.TerminationStatus()), MOI.get(o, Gurobi.ModelAttribute("MaxVio"))
