...
# small revision of loadflow_3
# adding Gurobi's SOCP
# 02/12/23
...
GRB_ENV = Gurobi.Env()
if true # The Relaxed SOCP -- OPF modeling
...
@warn "Initializing (Relaxed) OPF variables with PF solutions, MaxVio = $(maximum(values(vio_dict)))"
...
pdict = Dict(x => JuMP.value(x) for x in JuMP.all_variables(m)) # solution by Gurobi
JuMP.delete.(m, leqd)
JuMP.unregister(m, :leqd)
JuMP.@constraint(m, eqqd[b = 1:L], ckl[b]^2 + tkl[b]^2 == ckk[LD["from"][b]] * ckk[LD["to"][b]])
vio_dict = JuMP.primal_feasibility_report(m, pdict);
@warn "After strengthening `≤` to `==`, MaxVio = $(maximum(values(vio_dict)))" # 1.2115783500354915e-6
JuMP.@NLconstraint(m, thenl[b = 1:L], ckl[b] * sin(thakl[b]) + tkl[b] * cos(thakl[b]) == 0.)
vio_dict = JuMP.primal_feasibility_report(m, pdict);
@warn "After adding the sin-cos constraint, MaxVio = $(maximum(values(vio_dict)))" # 0.06757164189046574

JuMP.set_optimizer(m, Ipopt.Optimizer)
JuMP.optimize!(m)
@assert JuMP.termination_status(m) == JuMP.LOCALLY_SOLVED
@info "NLP OPF Optimal_ObjVal = $(JuMP.objective_value(m))" # 578.2359824446982
vio_dict = JuMP.primal_feasibility_report(m; atol = 1e-9);
@warn "After solving NLP OPF, MaxVio = $(maximum(values(vio_dict)))" # 1.0680219153513804e-8
