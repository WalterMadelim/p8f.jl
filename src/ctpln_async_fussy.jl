...
function bilin_expr(j, mj, iˈı::Function, β, μ, ν)
    pair = haskey(mj, :bLent)
    pBus, q = iˈı.(mj[:pBus]), iˈı.(mj[:q])
    prim_obj = iˈı(mj[:prim_obj])
    term_β = JuMP.@expression(mj, pair ? sum((pBus[t,1] + pBus[t,2])β[t] for t=1:T) : β ⋅ pBus)
    term_μ = JuMP.@expression(mj, pair ? sum((q[t,1] + q[t,2])μ[t] for t=1:T) : μ ⋅ q)
    term_ν = JuMP.@expression(mj, sum(q)ν)
    return JuMP.@expression(mj, +(prim_obj, term_β, term_μ, term_ν))
end;
...
const model, θ, β, μ, ν = initialize_out(an_UB);
...
# ⚠️ you cannot broadcast multiple 1:J tasks asyncly, since they read&write to the SAME inn[j]
function subproblemˈs_duty(j, snap, inbox) # the fussy version
    mj = inn[j]
    let (β, μ, ν) = (snap.β, snap.μ, snap.ν)
        JuMP.@objective(mj, Min, bilin_expr(j, mj, identity, β, μ, ν))
        JuMP.optimize!(mj)
        JuMP.assert_is_solved_and_feasible(mj; allow_local = false)
    end
    con = JuMP.@build_constraint(θ[j] ≤ bilin_expr(j, mj, ı, β, μ, ν))
    can_cut_off = function(new_snap)
        local β, μ, ν = new_snap.β, new_snap.μ, new_snap.ν
        local is_cut_off = new_snap.θ[j] > COT + bilin_expr(j, mj, ı, β, μ, ν)
    end
...
function masterˈs_algorithm(Δt)
    function shot!()
        ...
        local snap = (t = timestamp += 1, θ = ı.(θ), β = ı.(β), μ = ı.(μ), ν = ı(ν), ub = JuMP.objective_bound(model))
    end
    function masterˈs_loop()
        while true
            isempty(inbox) && (yield(); continue)
            js2reply, is_global_cut_off = Set{Int}(), false
            while !isempty(inbox)
                lens = @lock inbox_lock pop!(inbox)
                local j, is_cut_off, con = lens(snap)
                if is_cut_off
                    @lock model_lock JuMP.add_constraint(model, con)
                    print("\r▶ ub = $(snap.ub) | t = $(snap.t), j = $j")
                end
                push!(js2reply, j)
            ...
            if is_global_cut_off
                ...
                t0 = time()
            elseif time() - t0 > Δt
                println("\n▶▶▶ ub = $(snap.ub) | t = $(snap.t) Long time no improvements, quit!")
                return
            end
