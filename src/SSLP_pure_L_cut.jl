...
# lb = 59.6 = z_IP finally

    # julia> include("src/SSLP_pure_L_cut.jl")
    # Set parameter Username
    # Academic license - for non-commercial use only - expires 2024-08-09
    # [ Info: The perfect information LB z_PI = 35.800000000000004
    # [ Info: In master, lb = -59.80000000000006
    # [ Info: In master, lb = 43.20000000000001
    # [ Info: In master, lb = 46.428571428571445
    # [ Info: In master, lb = 47.0
    # [ Info: In master, lb = 51.460431654676256
    # [ Info: In master, lb = 59.60000000000001
    # [ Info: No more cut is generated in this iteration, ∀s ∈ S. Check if this current lb is well enough.
    # lb = 59.60000000000001
    # L_cut_num: [6, 6, 1, 6, 5]
    # [ Info: Leaving the loop on improving Q̂(s,x), ∀s ∈ S, in master()
...
function hatQ_initialization()::Vector{HatQs}
    ...
    # check this is a valid xt : Ax ≥ b
    if any(xt .< 0.) || any(xt .> 1.)
        @error "Gurobi gives an infeasible initial trial point! Please regenerate!"
    end
...
function algorithm1(s,hat_x,hat_θ_s) # there is also a global input hatE
    ...
    delta = .95 # strict[0, 1]early_terminating
    # --------------------------------- Basic Framework ---------------------------------
    o = silent_new_optimizer()
    ...
    while true # improving Q̂(s,x) for s in 1:scenes, using L-cut (13) generated by algorithm1
        ...
        for s in 1:scenes
            ...
            if pai' * xt + pai0 * thetat[s] <= cnst_L - 1e-6 # extended trial (xt, thetat[s]) violates the L-cut generated
                ...
                # @warn "At master_cut_generating: scene = $s, No violation, often revealing that Q̂($s,x) is already well enough w.r.t. the master obj"
            end
        end # end of for s in 1:scenes
        if !new_cut_gened
            @info "No more cut is generated in this iteration, ∀s ∈ S. Check if this current lb is well enough."
            println("lb = $lb")
            println("L_cut_num: $L_cut_num")
            @info "Leaving the loop on improving Q̂(s,x), ∀s ∈ S, in master()"
            break
        end
    end # end of while
