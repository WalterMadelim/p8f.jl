...
T = 6  | ([2.43250, 2.43250], 26s)  ([4.64843, 4.67767], 147s)  ([4.91033, 4.91033], 1388s)     not valid after 30min
T = 8  | ([8.67519, 8.76274], 85s)  ([23.9188, 24.1423], 229s)  ([13.3761, 474.702], 30min)     not valid after 30min
———————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
...
btBnd = 135.0 # improper this value could incur infeasibility of f()
...
function gencut_ℶ2(Z, yM, iY, of, u, v, x) # decorator
    cn, pu, pv, px, pY = gencut_f_uvxY(Z, yM, iY, u, v, x)
    pβ2 = -Z
...
function gencut_f_uvxY(Z, yM, iY, uΓ, vΓ, xΓ)
    YΓ = yM[:, :, iY]
    ø = JumpModel(1)
    @addMatVarViaCopy(u, uΓ)
    @addMatVarViaCopy(v, vΓ)
...
function tryPush_ℶ2(Z, yM, iY, oℶ2, x, β2)::Bool # 👍 use this directly
    u, v = decode_uv_from_x(x)
    cn, pu, pv, px, pY, pβ2 = gencut_ℶ2(Z, yM, iY, NaN, u, v, x)
