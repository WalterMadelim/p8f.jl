...
import Gurobi
                ...
                function Q_star(s,pai,pai0)
                    if pai0 == 0.0
                        @warn "At input of Q_star, π0 == 0.0"
                    elseif pai0 < 0.0 
                        @error "At input of Q_star, π0 should be ≥ 0.0"
                    end
                    o = silent_new_optimizer()
                ...
                function algorithm1(s,hat_x,hat_θ_s) # there is also a global input hatE
                    ...
                    while true
                        ...
                        if ub <= 1e-6 * (abs(hat_θ_s) + 1.)
                            @warn "leave algorithm1 due to hopeless to find a strong violated cut."
                            break
                        end
...
function master()
    ...
    while true # updating Q̂(s,x) for s in 1:scenes, using L-cut (13) generated by algorithm1
        ...
        if status != MOI.OPTIMAL
            ...
            if lb > 59
                println("lb is well enought to stop the master iteration!")
                println("lb = $lb")
        ...
        for s in 1:scenes
            ...
            elseif pai0 == 0.
                @error "Going to generate a feasibility cut ??? please check why pai0 derived here == 0.0"
            end
            if pai' * xt + pai0 * thetat[s] <= cnst_L - 1e-6 # extended trial (xt, thetat[s]) violates the L-cut generated
                ...
                @warn "a newly generated (13) L-cut is not violated, please check !!!!!"
            end
        end
    end
end
...
# # write algorithm 1 where test scene = 1
# s = 1
# xt = [1.,0.,1.]
# theta = -50.
